...
export function repeatPattern(pattern, times) {
  if (typeof pattern !== "string" || Number.isInteger(times) == false || times < 0 || typeof times !="number") return "";
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  // assuming string below
  const splitted = colorString.split(oldSep); // array of colours
  return splitted.join(newSep);
}
